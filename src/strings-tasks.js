...
function removeFirstOccurrences(str, value) {
  ...
  const newStr = str.slice(0, index) + str.slice(index + value.length);
  return index === -1 ? str : newStr;
}
...
function isPalindrome(str) {
  const normalizedStr = str
    ...
    .replaceAll(' ', '')
    .replace(/[!?,]/g, '');
  const normalizedReversedStr = normalizedStr.split('').reverse().join('');
  return normalizedStr === normalizedReversedStr;
}
